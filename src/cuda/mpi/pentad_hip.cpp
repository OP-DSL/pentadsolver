...
#include <hip/hip_cooperative_groups.h> // for this_grid, grid_group
...
void gpsv_batched_forward(const Float *ds, const Float *dl, const Float *d,
  ...
  if (t_solvedim == 0) {
    hipLaunchKernelGGL(gpsv_forward_batch_x_kernel, nblocks, block_dim_x, 0, 0, 
        ds, dl, d, du, dw, x, dss, dll, duu, dww, comm_buf, snd_bottom_buf,
        t_n_sys, t_dims[t_solvedim]);
  } else if (t_solvedim == t_ndims - 1) {
    hipLaunchKernelGGL(gpsv_forward_batch_outermost_kernel, nblocks, block_dim_x, 0, 0, 
        ds, dl, d, du, dw, x, dss, dll, duu, dww, comm_buf, snd_bottom_buf,
        t_n_sys, t_dims[t_solvedim]);
  } else {
    size_t n_sys_in =
    ...
    hipLaunchKernelGGL(gpsv_forward_batch_middle_kernel, nblocks, block_dim_x, 0, 0, 
        ds, dl, d, du, dw, x, dss, dll, duu, dww, comm_buf, snd_bottom_buf,
        n_sys_in, n_sys_out, t_dims[t_solvedim]);
  }
  CHECK_CUDA(hipPeekAtLastError()); // NOLINT
}
// ----------------------------------------------------------------------------
// Backward
// ----------------------------------------------------------------------------
...
void gpsv_batched_backward(const Float *dss, const Float *dll, const Float *duu,
  ...
  int nblocks               = 1 + (static_cast<int>(t_n_sys) - 1) / block_dim_x;
  if (t_solvedim == 0) {
    hipLaunchKernelGGL(gpsv_backward_batch_x, nblocks, block_dim_x, 0, 0, 
        dss, dll, duu, dww, x, x_top, xp1, t_n_sys, t_n_sys,
        t_dims[t_solvedim]);
  } else if (t_solvedim == t_ndims - 1) {
    hipLaunchKernelGGL(gpsv_backward_batch_outermost, nblocks, block_dim_x, 0, 0, 
    ...
    hipLaunchKernelGGL(gpsv_backward_batch_middle, nblocks, block_dim_x, 0, 0, 
...
void send_rows_to_nodes(pentadsolver_handle_t params, int t_solvedim,
  ...
  if (leftrank >= 0 || rightrank < nproc) {
    hipMemcpy(sndbuf_h, sndbuf_d, msg_size * sizeof(Float),
  ...
  if (leftrank >= 0 && (static_cast<unsigned>(dir) &
    ...
    hipMemcpy(rcvbufL_d, rcvbufL_h, msg_size * sizeof(Float),
               hipMemcpyHostToDevice);
  }
  if (rightrank < nproc && (static_cast<unsigned>(dir) &
                            static_cast<unsigned>(communication_dir_t::UP))) {
    hipMemcpy(rcvbufR_d, rcvbufR_h, msg_size * sizeof(Float),
               hipMemcpyHostToDevice);
  }
#endif
}
...
void eliminate_bottom_rows_from_reduced(pentadsolver_handle_t params,
  ...
  hipLaunchKernelGGL(eliminate_bottom_row_kernel, nblocks, block_dim_x, 0, 0, rank, t_n_sys, rcvbuf_d,
                                                        top_buf_d);
  CHECK_CUDA(hipPeekAtLastError()); // NOLINT
}
...
inline void solve_reduced_pcr(pentadsolver_handle_t params, Float *rcvbuf_d,
  ...
  for (int p = 0; p < P; ++p) {
    ...
    // PCR algorithm
    hipLaunchKernelGGL(pcr_iteration_kernel, nblocks, block_dim_x, 0, 0, 
...
inline void solve_reduced_jacobi(pentadsolver_handle_t params, Float *rcvbuf_d,
  ...
  do {
    send_rows_to_nodes<communication_dir_t::ALL, Float>(
        ...
        rcvbufL_d, rcvbufL_h, rcvbufR_d, rcvbufR_h);
    // kernel
    hipLaunchKernelGGL(jacobi_iteration_kernel, nblocks, block_dim_x, 0, 0, 
  ...
  hipMemcpy(sndbuf_d + t_n_sys * 2 * 4, x_cur_d,
...
void pentadsolver_gpsv_batch(pentadsolver_handle_t params, const Float *ds,
  ...
  CHECK_CUDA(hipDeviceSynchronize()); // NOLINT
  auto t2 = clock::now();
